ProcMeshController = class(ProcMeshController)
local M = ProcMeshController

Vector3ArrayClass = class(Vector3ArrayClass)
...
function Vector3Array.__add(a, b)
    return Vector3Array.add(a, b)
end
...
function Vector3Array.__sub(a, b)
    return Vector3Array.sub(a, b)
end
...
function Vector3Array.__mul(a, b)
    return Vector3Array.mul(a, b)
end
...
function Vector3Array.__div(a, b)
    return Vector3Array.div(a, b)
end
...
local function menger_sponge(iterations, vmin, vmax, vbuffer, ibuffer)
    ...
    if iterations == 0 then
        ...
        append_f(ibuffer, i, i + 1, i + 2, i, i + 2, i + 3)
        i = i + 4
        append_f(ibuffer, i, i + 1, i + 2, i, i + 2, i + 3)
        i = i + 4
        append_f(ibuffer, i, i + 1, i + 2, i, i + 2, i + 3)
...
local function make_patch_sphere(sphere_points, r, d, vbuffer, ibuffer)
    ...
    if ibuffer then
        ...
        for n = 0, count - 1 do
            local i = 24 * n
            ibuffer:push(i, i + 1, i + 2, i, i + 2, i + 3)
            i = i + 4
            ibuffer:push(i, i + 1, i + 2, i, i + 2, i + 3)
            i = i + 4
            ibuffer:push(i, i + 1, i + 2, i, i + 2, i + 3)
...
function M:start()
    ...
    -- local unit =
    --     stingray.World.spawn_unit(stingray.Application.main_world(), "core/units/camera", stingray.Vector3(-10, 0, 5))
    -- self.sphere =
    --     self:make_procmesh(
    --     unit,
    --     "rp_root",
    --     vbuffer,
    --     ibuffer,
    --     {vertex_buffer_validity = stingray.RenderBuffer.RB_VALIDITY_UPDATABLE}
    -- )
    -- table.insert(self.spinning_units, unit)
...
function M:make_procmesh(unit, node, vertices, indices, options)
    ...
    t.vbuffer =
        stingray.RenderBuffer.create_buffer(
        options.vertex_buffer_validity or stingray.RenderBuffer.RB_VALIDITY_STATIC,
        stingray.RenderBuffer.RB_VERTEX_BUFFER_VIEW,
        6 * 4,
        vertices
    )
    t.ibuffer =
        stingray.RenderBuffer.create_buffer(
        stingray.RenderBuffer.RB_VALIDITY_STATIC,
    ...
    t.mesh =
        stingray.ProceduralMesh.create(
        unit,
        node,
        "procedural-mesh",
        stingray.ProceduralMesh.MO_VIEWPORT_VISIBLE_FLAG + stingray.ProceduralMesh.MO_SHADOW_CASTER_FLAG
    )

    stingray.ProceduralMesh.set_bounding_box(
        t.mesh,
        stingray.Vector3(-100, -100, -100),
        stingray.Vector3(100, 100, 100)
    )
    local batches = {
        {
...
function M:update(dt)
    ...
    if self.sphere then
        ...
        stingray.RenderBuffer.update_buffer(
            self.sphere.vbuffer,
            stingray.RenderBuffer.RB_FLOAT_COMPONENT,
            self.sphere_vbuffer
        )
    end
end
